...
Circle Gateway connects multiple blockchains, allowing you to move USDC between them and pay for resources gas free.

## 1. Gateway Networks (Gas Free Payments, Deposits & Withdrawals)

All Gateway-supported EVM chains support **gas free batched payments** using this SDK. You can deposit USDC on any of these chains and make gas free payments on the **same chain**.

> **Important:** Deposits and payments must be on the same chain. If you deposit on Arc Testnet, you pay on Arc Testnet. To avoid deposit wait times on slow-finality chains, consider using [Circle BridgeKit](https://developers.circle.com/circle-mint/docs/bridgekit-quickstart) to bridge USDC to a faster chain first.
